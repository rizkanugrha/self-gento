...
export async function SnapSave(url) {
    try {
        ...
        const results = [];
        const addedUrls = new Set(); // Set untuk melacak URL yang sudah ditambahkan
        ...
        if ($("table.table").length || $("article.media > figure").length) {
            ...
            $("tbody > tr").each((_, el) => {
                ...
                // Periksa apakah URL sudah ada dalam Set
                if (!addedUrls.has(_url)) {
                    addedUrls.add(_url); // Tambahkan URL ke Set
                    results.push({
...
// (async () => {
//     try {
//         const results = await SnapSave('https://www.instagram.com/p/DCWXlpzPlRK/?utm_source=ig_web_copy_link');
//         console.log(results);
//     } catch (error) {
