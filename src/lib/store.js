...
class InMemoryStore extends events.EventEmitter {
    ...
    readFromFile(filename = 'store.json') {
        if (fs.existsSync(filename)) {
            const data = JSON.parse(fs.readFileSync(filename, 'utf-8'))
            this.load(data)
            this.logger.info(`Store loaded from ${filename}`)
        } else {
            this.logger.warn(`Store file ${filename} not found`)
        }
    }
    ...
    setPresence(chatId, presence) {
        ...
        if (presence && presence.participant) {
            ...
            this.logger.warn(`Presence set without participant: ${JSON.stringify(presence)}`)
        }
    }
    ...
    updatePresence(chatId, presence) {
        ...
        if (presence && presence.participant) {
            ...
            this.logger.warn(`Presence update without participant: ${JSON.stringify(presence)}`)
        }
    }
    ...
    bind(ev) {
        ev.on('contacts.set', this.setContacts.bind(this))
        ...
        ev.on('chats.set', this.setChats.bind(this))
        ...
        ev.on('sticker-packs.set', this.setStickerPacks.bind(this))
        ev.on('sticker-packs.upsert', (packs) => packs.forEach(this.upsertStickerPack.bind(this)))

        ev.on('auth-state.update', this.setAuthState.bind(this))
        ev.on('history-sync.completed', this.markHistorySynced.bind(this))
    }
}
