...
logger.level = 'fatal'; // Level fatal agar log bersih dan cepat
...
if (used > limit) {
    console.warn('[MEMORY] Limit reached, restarting...');
    process.exit(1);
}
    ...
    setInterval(() => {
         fs.readdir(pathSession, (err, files) => {
             if (err) return;
             files.filter(file => file.startsWith('pre-key') || file.startsWith('sender-key') || file.startsWith('session-'))
                 .forEach(file => fs.unlink(`${pathSession}/${file}`, () => { }));
         });
     }, 60000 * 15); 
